...
export const PgV4SimpleSubscriptionsPlugin = extendSchema((build) => {
  ...
  console.log({ nodeIdHandlerByTypeName });
  return {
    typeDefs: [
      gql`
                  ...
                  (node, nodeIdFromEvent, nodeIdHandlerByTypeName) =>
                    ($event) => {
                      ...
                      console.log(
                        "relatedNode",
                        { nodeIdHandlerByTypeName },
                        node(nodeIdHandlerByTypeName, $nodeId)
                      );
...
const nodeObjToNodeId = EXPORTABLE(
  () =>
    function nodeObjToNodeId(obj: any[] | undefined): string | null {
      if (!obj) return null;
      const nodeId = Buffer.from(JSON.stringify(obj), "utf8").toString(
        "base64"
      );
      console.log("nodeObjToNodeId", obj, nodeId);
      return nodeId;
    },
  [],
  "nodeObjToNodeId"
